...
export default function StoragePage() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await uploadSubjectFile(currentSubjectId!, lessonIdParam, formData);
  ...
  const handleDeleteFile = async (fileId: string) => {
    ...
    try {
      await deleteSubjectFile(currentSubjectId!, fileId);
    ...
    <>
      ...
      <div className="flex-grow overflow-y-auto no-scrollbar pr-1 pb-6 flex flex-col gap-6 text-left select-none w-full">
        <div className="w-full px-2 md:px-4 flex flex-col gap-6">
          ...
          <div className="mt-2 text-left animate-in fade-in duration-300">
            <h1 className="text-lg sm:text-xl font-black text-zinc-955 tracking-tight">Files & Storage</h1>
          </div>

          {/* Side-by-Side Layout: File Explorer & Storage Limit Card */}
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 w-full items-start mt-2">
            
            {/* Left Column: Directory Explorer (takes 8 cols on desktop) */}
            <div className="lg:col-span-8 flex flex-col gap-5">
              ...
              <div className="w-full flex flex-col sm:flex-row items-center justify-between gap-3 mb-1">
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto">
                  ...
                  {/* Folder Options Menu Tooltip */}
                  <div className="relative group/menu shrink-0">
                    <button className="w-9 h-9 rounded-full bg-white border border-[#EFECE6] flex items-center justify-center text-zinc-500 hover:text-zinc-800 hover:bg-zinc-50 transition-all active:scale-95 shadow-2xs cursor-pointer" title="Folder actions">
                      <FolderOpen className="w-4 h-4" />
                    </button>
                    {/* Tooltip Content: create folder, upload file, clear all the folder */}
                    <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 bg-zinc-955 text-white rounded-xl p-2.5 shadow-lg z-50 flex flex-col gap-1.5 text-[10px] font-bold border border-zinc-800 opacity-0 scale-95 pointer-events-none group-hover/menu:opacity-100 group-hover/menu:scale-100 transition-all duration-150 w-36 text-left">
                      ...
                      <div className="flex items-center gap-1.5 text-zinc-300 border-t border-zinc-800 pt-1.5 mt-0.5">
                        <span className="w-1.5 h-1.5 rounded-full bg-rose-500" />
                        <span>Clear all the folder</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              >
                  ...
                  <div className="flex flex-col gap-5">
                            ...
                            <div key={file.id} className="px-5 py-4 flex items-center justify-between gap-4 hover:bg-zinc-50/50 transition-colors group">
                              ...
                              <div className="flex items-center gap-1 shrink-0 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
                                <button
                                  ...
                                  className="w-8 h-8 rounded-lg hover:bg-red-50 flex items-center justify-center text-zinc-400 hover:text-red-600 transition-colors cursor-pointer"
                                  title="Delete File"
                                >
                    ...
                    <div className="flex flex-col gap-3">
                      {currentSubject.modules.length === 0 ? (
                        <div className="w-full h-48 bg-white border border-[#EFECE6] rounded-[32px] flex items-center justify-center text-zinc-400 text-xs">
                          No modules found in this subject
                        </div>
                      ) : (
                        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                          {currentSubject.modules.map((mod) => {
                            const modFilesCount = mod.lessons.reduce((acc, les) => acc + (les.files || []).length, 0);
                            ...
                            return (
                              <FolderCard
                                key={mod.id}
                                name={mod.title}
                                itemCount={modFilesCount}
                                subtitle="Module Directory"
                                leftStatNumber={modFilesCount < 10 ? `0${modFilesCount}` : modFilesCount}
                                leftStatLabel="Doc"
                                rightStatNumber={mod.lessons.length}
                                rightStatLabel="Lessons"
                                bannerColor="#FAF7F2"
                                bannerHasStripes={false}
                                onClick={() => navigateTo(`${currentSubject.id}/${mod.id}`)}
                                onContextMenu={(e) => {
                                  e.preventDefault();
                                  e.stopPropagation();
                                  setContextMenu({
                                    ...
                                    y: e.clientY,
                                    type: "folder",
                                    folderPath: `${currentSubject.id}/${mod.id}`,
                                  });
                                }}
                              />
                            );
                ...
                {/* 3. MODULE LEVEL: DISPLAY LESSON FOLDERS */}
                {currentSubject && currentModule && !currentLesson && (
                  <div className="flex flex-col gap-3">
                    {currentModule.lessons.length === 0 ? (
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                        {currentModule.lessons.map((les) => {
                          const lesFilesCount = (les.files || []).length;
                          ...
                          return (
                            <FolderCard
                              key={les.id}
                              name={les.title}
                              itemCount={lesFilesCount}
                              subtitle="Lesson Directory"
                              leftStatNumber={lesFilesCount < 10 ? `0${lesFilesCount}` : lesFilesCount}
                              leftStatLabel="Doc"
                              rightStatNumber={undefined}
                              rightStatLabel={undefined}
                              bannerColor="#FAF7F2"
                              bannerHasStripes={false}
                              onClick={() => navigateTo(`${currentSubject.id}/${currentModule.id}/${les.id}`)}
                              onContextMenu={(e) => {
                                e.preventDefault();
                                e.stopPropagation();
                                setContextMenu({
                                  ...
                                  y: e.clientY,
                                  type: "folder",
                                  folderPath: `${currentSubject.id}/${currentModule.id}/${les.id}`,
                                });
                              }}
                            />
                          );
                ...
                {/* 4. LESSON LEVEL: DISPLAY LESSON ATTACHMENTS */}
                {currentSubject && currentModule && currentLesson && (
                  <div className="flex flex-col gap-3">
            ...
            {/* Right Column: Storage Limit Card (takes 4 cols on desktop) */}
            <div className="lg:col-span-4 w-full sticky top-4 shrink-0 animate-in fade-in duration-300">
              {loadingStats ? (
                <div className="flex flex-col gap-1.5 animate-pulse bg-white border border-[#EFECE6] p-6 rounded-[32px] h-[240px] shadow-3xs">
      ...
      {contextMenu.visible && (
        <div 
          ...
          className="fixed bg-zinc-950/95 backdrop-blur-md text-white rounded-2xl p-2 shadow-2xl z-[100] flex flex-col gap-1 text-[11px] font-bold border border-zinc-800/80 animate-in fade-in zoom-in-95 duration-100 min-w-[150px]"
          ...
          {contextMenu.type === "container" ? (
            <>
              <button 
                onClick={() => {
                  ...
                  showToast("Create folder action triggered", "success");
                }}
                className="flex items-center gap-2.5 px-3 py-2 hover:bg-zinc-850 rounded-lg text-left text-zinc-300 hover:text-white transition-colors cursor-pointer w-full"
              >
                <span className="w-1.5 h-1.5 rounded-full bg-blue-500" />
                <span>Create folder</span>
              </button>
              ...
              <button 
                onClick={() => {
                  setContextMenu(prev => ({ ...prev, visible: false }));
                  showToast("Clear all folders action triggered", "success");
                }}
                className="flex items-center gap-2.5 px-3 py-2 hover:bg-red-950/40 hover:text-red-300 rounded-lg text-left text-zinc-400 transition-colors cursor-pointer w-full"
              >
                <span className="w-1.5 h-1.5 rounded-full bg-rose-500" />
                <span>Clear all the folder</span>
              </button>
            ...
            <>
              <button 
                onClick={() => {
                  ...
                  if (contextMenu.folderPath) {
                    navigateTo(contextMenu.folderPath);
                  }
                }}
                className="flex items-center gap-2.5 px-3 py-2 hover:bg-zinc-850 rounded-lg text-left text-zinc-300 hover:text-white transition-colors cursor-pointer w-full"
              >
              ...
              <button 
                onClick={() => {
                  setContextMenu(prev => ({ ...prev, visible: false }));
                  showToast("Clear folder action triggered", "success");
                }}
                className="flex items-center gap-2.5 px-3 py-2 hover:bg-red-950/40 hover:text-red-300 rounded-lg text-left text-zinc-400 transition-colors cursor-pointer w-full"
              >
                <span className="w-1.5 h-1.5 rounded-full bg-rose-500" />
